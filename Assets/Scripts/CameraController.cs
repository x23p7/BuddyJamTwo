...
public class CameraController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        {
            Vector3 targetPosition2D = Vector3.Scale(target.transform.position, Vector3.right) + targetCam.transform.position.y * Vector3.up + Vector3.forward * targetCam.transform.position.z;
        ...
        {
            Vector3 targetPosition2D = targetCam.transform.position.x * Vector3.right + Vector3.Scale(target.transform.position, Vector3.up) + Vector3.forward * targetCam.transform.position.z;
            targetCam.transform.position = Vector3.Lerp(targetCam.transform.position, targetPosition2D, followLerpSpeed * Time.fixedDeltaTime);
        }
    }
}
